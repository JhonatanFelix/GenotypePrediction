...
The workflow is:
    ...
    def fit(self, X:np.ndarray, feature_names: list[str] | None = None) -> "GenotypeStandardizer":
        ...
        if contain_nan:
            print("The array X contain missing values, and they're going to be substituted by the mean of non-missing values.")
        ...
        if not np.any(keep_mask):
            raise ValueError("All genotype columns have zero variance after imputation.")
        ...
        if feature_names is None:
            ...
            if len(feature_names) != X.shape[1]:
                raise ValueError("features_names must match the number of genotype columns.")
    ...
    def transform(self, X: np.ndarray) -> np.ndarray:
        ...
        if self.keep_mask_ is None or self.marker_means_ is None or self.marker_stds_ is None:
            raise RuntimeError("The standardizer must be fitted before calling transform.")

        X = np.asarray(X, dtype=float)
        if X.ndim != 2:
            raise ValueError("X must be a 2D array.")
        if X.shape[1] != self.keep_mask_.shape[0]:
            raise ValueError("X has a different number of comumns than the fitted training data.")
    ...
    def center_y(self, y: np.ndarray) -> np.ndarray:
        ...
        if self.y_mean_ is None:
            raise RuntimeError("The phenotype mean is not available. Call fit_y first please")
        y = np.asarray(y, dtype=float)
