"""BayesC with fixed global inclusion probability ``q``."""
...
from genotypeprediction.inference.gibbs import run_bayesc_fixed_q_gibbs
...
class BayesCFixedQ:
    """Spike-and-slab genomic regression with user-specified fixed ``q``."""
    ...
    def fit(
        ...
        posterior = run_bayesc_fixed_q_gibbs(
            X=X_standardized,
            y=y_centered,
            q=self.q,
            n_iter=self.n_iter,
    ...
    def get_posterior_summary(self) -> dict[str, np.ndarray | float | list[float]]:
        ...
        return {
            ...
            # Backward-compatible aliases used by the Milestone 2 scripts/tests.
